...
TTS_URL = 'http://localhost:8020'  # optional local TTS server (e.g. a Piper/XTTS HTTP wrapper); used as a fallback if Fish Audio S2 isn't available
...
FISH_VOICE_REFERENCE_PATH = os.environ.get(
    'FISH_VOICE_REFERENCE_PATH',
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'weng_voice.wav'))
...
_FISH_VOICES_CACHE = {'voices': None, 'source': None, 'error': None, 'fetched_at': 0}
_FISH_VOICES_CACHE_TTL = 60  # seconds; avoids hammering the API every time the dropdown opens

def fish_audio_list_voices(force=False):
    """List voices available for narration: registered voice models (from Fish
    Audio's cloud API, if FISH_AUDIO_API_KEY is set) plus the always-available
    default clone from FISH_VOICE_REFERENCE_PATH. Self-hosted servers typically
    don't expose a model-listing endpoint, so in that case (no API key) we just
    return the default clone entry. Returns (voices, source, error) where source
    is 'cloud' | 'self_hosted_default' | 'error', and each voice is
    {'id': <reference_id or ''>, 'title': <display name>, 'languages': [...]}."""
    now = time.time()
    if not force and _FISH_VOICES_CACHE['voices'] is not None and \
            now - _FISH_VOICES_CACHE['fetched_at'] < _FISH_VOICES_CACHE_TTL:
        return _FISH_VOICES_CACHE['voices'], _FISH_VOICES_CACHE['source'], _FISH_VOICES_CACHE['error']

    default_voice = {'id': '', 'title': 'Cloned from weng_voice.wav (default)', 'languages': []}
    ...
    if FISH_AUDIO_API_KEY:
        ...
        # there (404/refused/anything else), we just keep the default clone —
        # this is expected for a plain self-hosted instance, not an error.
        ...
        try:
            ...
            pass  # no listing endpoint on this server — default clone only, not an error

    _FISH_VOICES_CACHE.update(voices=voices, source=source, error=error, fetched_at=now)
...
def generate_tts(text, output_wav_path, rate=175, voice_id=None, reference_audio_path=FISH_VOICE_REFERENCE_PATH, language=None):
    """Generate a narration WAV from text. Tries Fish Audio's S2 API first (auto-detects
    language, including Tagalog, unless `language` overrides that), then falls back to
    an optional local TTS HTTP server if Fish Audio isn't available. Returns
    (ok, error_message). By default the narration voice is cloned from
    FISH_VOICE_REFERENCE_PATH (weng_voice.wav); pass an explicit `voice_id` to use a
    pre-registered reference_id instead."""
    text = (text or '').strip()
    if not text:
        return False, 'No text provided'
    fish_err = None
    try:
        ok, err = fish_audio_tts(text, output_wav_path, voice_id=voice_id, rate=rate,
                                  reference_audio_path=reference_audio_path, language=language)
        if ok:
            return True, None
        fish_err = err
        print(f'Fish Audio TTS error: {err}')
    except Exception as e:
        fish_err = str(e)
        print(f'Fish Audio unavailable, falling back: {e}')
    local_err = None
    try:
        r = requests.post(f'{TTS_URL}/tts', json={'text': text, 'voice': voice_id, 'rate': rate}, timeout=3)
        if r.ok and r.content:
            with open(output_wav_path, 'wb') as f:
                f.write(r.content)
            if os.path.exists(output_wav_path) and os.path.getsize(output_wav_path) > 0:
                return True, None
        local_err = f'local TTS server returned HTTP {r.status_code}'
    except Exception as e:
        local_err = f'local TTS server unreachable ({e})'
    return False, f'Fish Audio: {fish_err or "no response"} | {local_err}'
...
def api_trailer():
    ...
    params = dict(path=path, orig_name=orig_name, mode=mode, genre=genre, scoring_mode=scoring_mode,
                  trailer_length=trailer_length, max_scene_dur=max_scene_dur, transition=transition, xfade_dur=xfade_dur,
                  target_loudness=target_loudness, true_peak=true_peak, beat_match=beat_match, broadcast_stereo=broadcast_stereo, model=model,
                  ...
                  vo_language=vo_language, vo_ref_upload_path=vo_ref_upload_path,
...
def api_health():
    ...
    (Ollama, Fish Audio S2, faster-whisper, ACE-Step, Woosh, and the optional
    TTS fallback), checked in parallel so one slow/dead service doesn't stall
    the others. Returns per-service status plus an overall ok flag."""
    checks = [
...
@app.route('/api/fish_audio/voices')
def api_fish_audio_voices():
    """Lists narration voices and languages for the Narration dropdowns. Voices:
    the default weng_voice.wav clone, plus any registered voice models Fish
    Audio's cloud API reports for this account (only populated when
    FISH_AUDIO_API_KEY is set — a plain self-hosted server has no models to
    list beyond the default clone). Languages: the curated override list;
    'auto' is always first and is the recommended choice."""
    force = request.args.get('refresh') == '1'
    voices, source, error = fish_audio_list_voices(force=force)
...
def api_vo_preview():
    ...
    ref_path = FISH_VOICE_REFERENCE_PATH
    ...
    ok, err = generate_tts(text, out_path, rate=rate, voice_id=voice_id,
                            reference_audio_path=ref_path, language=language)
    if not ok:
...
def _run_trailer_job(jid, params):
    ...
    scene_list = sm.get_scene_list()
    if not scene_list:
        job_set(jid, error='No scene changes detected. Try a video with clear cuts.')
        return
    ...
    for seg_i, seg in enumerate(selected):
        ...
        r = subprocess.run([FFMPEG, '-y', '-ss', str(seg.get('trim_start', seg['start'])), '-i', path,
    ...
    if scoring_audio_path and total_sel > 0:
        ...
        if prepared_bgm:
            ...
            # flat volume multiplier alone won't correct for that. Normalize to a
            # level a fixed amount under dialogue, *then* duck it under SOT on top.
            bgm_target = target_loudness - 8
    ...
    if vo_mode != 'none':
        if vo_mode == 'tts':
            if vo_ref_upload_path:
                vo_step_note = ' (cloning voice from uploaded reference sample)'
            elif vo_voice:
                vo_step_note = f' (voice: {vo_voice})'
            else:
                vo_step_note = ' (cloning voice from weng_voice.wav)'
        else:
            vo_step_note = ''
        ...
        if vo_mode == 'upload' and vo_upload_path and os.path.exists(vo_upload_path):
            ...
            tts_kwargs = {'rate': vo_rate, 'voice_id': vo_voice, 'language': vo_language}
...
<div class="hdr"><div class="hdr-inner">
  ...
  <div class="hdr-engines">opencv &middot; pyscenedetect &middot; ai vision &middot; ffmpeg &middot; ace-step &middot; woosh &middot; fish audio s2 &middot; faster-whisper</div>
</div></div>
<div class="container">
<div class="tabs">
...
<div id=tr-progress-area class=no-data style="display:none;text-align:left;padding:20px 24px">
  ...
  <button type=button id=tr-cancel-btn class=btn style="margin-top:8px;background:var(--danger,#c94f4f)">Cancel</button>
</div>
<form id=tf method=POST action=/api/trailer/generate enctype=multipart/form-data>
  ...
  <div class="genre-manual adv-only" style="display:none">
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:8px 0">
    ...
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0 0 0 12px;cursor:pointer">
      <input type=checkbox name=sync_beats> Sync cuts to beat
    </label>
  </div>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin:8px 0">
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
      <input type=radio name=scoring_mode value=generate data-requires="ace_step"> Generate music
    </label>
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
      <input type=radio name=scoring_mode value=none checked> None
    </label>
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin:8px 0">
    ...
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
      <input type=radio name=vo_mode value=tts data-requires="fish_audio,tts_fallback"> Narration (Fish Audio voice clone)
    </label>
  </div>
  <p id=vo-gating-note style="display:none;margin-top:-4px;margin-bottom:8px;font-size:12px;color:var(--amber)">"Narration (Fish Audio voice clone)" is unavailable — Fish Audio and the TTS fallback aren't reachable.</p>
  ...
  <div id=vo-tts-area style="display:none">
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin:8px 0 4px">
      <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
        <input type=radio name=vo_voice_source value=default checked> Default clone (<code>weng_voice.wav</code>)
      </label>
      <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
        <input type=radio name=vo_voice_source value=upload> Upload a reference sample
      </label>
      <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
        <input type=radio name=vo_voice_source value=registered> Choose a voice
      </label>
    </div>
    ...
    <div id=vo-registered-voice-area style="display:none;margin-bottom:6px">
    ...
    <p style="margin-top:4px;font-size:12px;opacity:.75">Uses Fish Audio S2 voice cloning (auto-detects the script's language, including Tagalog, unless overridden above). Points at your self-hosted server by default (no API key needed); set FISH_AUDIO_URL/FISH_AUDIO_API_KEY env vars to use the hosted cloud API instead — voices you've registered there populate the "Choose a voice" dropdown.</p>
  ...
  // Populate the "Choose a voice" and "Language" dropdowns from Fish Audio.
  // Cached client-side per page load; call loadFishAudioVoices(true) to refresh.
  var _fishVoicesLoaded = false
  function loadFishAudioVoices(force){
    if(_fishVoicesLoaded && !force) return
    _fishVoicesLoaded = true
    fetch('/api/fish_audio/voices' + (force ? '?refresh=1' : '')).then(r=>r.json()).then(function(d){
      ...
      if(voiceSel.options.length === 0){
        var opt = document.createElement('option')
        opt.value = ''; opt.textContent = 'No registered voices found'
        voiceSel.appendChild(opt)
        note.textContent = d.source === 'self_hosted_default'
          ? 'Set FISH_AUDIO_API_KEY to list voices registered on the Fish Audio cloud API.'
          : (d.error || '')
      } else {
        note.textContent = d.source === 'cloud' ? (voiceSel.options.length + ' voice(s) from your Fish Audio account') : ''
      }
  ...
  <button class=btn type=submit id=trailer-submit-btn style="margin-top:22px">Generate Episodic Promo Plug</button>
</form>
</div>
...
<p style="font-size:12px;opacity:.8">Pings Ollama, Fish Audio S2, faster-whisper, ACE-Step, Woosh, and the optional TTS fallback and reports whether each is reachable, with response latency. Doesn't call any generation endpoints, so it's safe (and cheap) to run anytime.</p>
...
<p><strong>Narration (Fish Audio S2 voice clone)</strong>:</p>
<pre>POST /api/trailer/generate  + vo_mode=tts &amp; vo_text=&lt;script&gt;
GET  /api/fish_audio/voices           list registered voices + languages
POST /api/vo/preview  + text, rate, language, voice or ref_upload</pre>
<p style="font-size:12px;opacity:.8">
By default, narration uses Fish Audio voice cloning: the reference sample is read from
<code>FISH_VOICE_REFERENCE_PATH</code> (defaults to <code>weng_voice.wav</code> next to
this script) and sent to Fish Audio as a <code>references</code> entry so speech is
generated in that cloned voice — no pre-registered <code>reference_id</code> needed.
The Narration section instead lets you pick a registered voice (populated from
<code>/api/fish_audio/voices</code>, requires <code>FISH_AUDIO_API_KEY</code> for the
cloud API), upload your own reference sample to clone on the fly, or override the
auto-detected language — and generate a short preview via
<code>/api/vo/preview</code> before running the full trailer job. Configured via env vars:
</p>
<div class=card>
<div class=info>
  ...
  <div class=info-item><strong>FISH_VOICE_REFERENCE_PATH</strong><br><small>weng_voice.wav</small></div>
</div>
</div>
</div>
...
<ol style="line-height:1.9">
  <li><strong>Scene detection</strong> — PySceneDetect (ContentDetector) finds every cut in the source video.</li>
  <li><strong>Scoring</strong> — each scene is scored on sharpness/brightness/duration (OpenCV), boosted by AI Vision's 1-5 quality rating, and boosted further for scenes with quotable dialogue when VISION + STT is selected (faster-whisper transcription).</li>
  ...
  <li><strong>Scene selection</strong> — highest-scoring scenes are picked to fill the target duration, respecting min/max clip length, snapping cut points to the beat grid and/or word boundaries when enabled, and topping up across a few passes to land within ~0.5s of the target length.</li>
  <li><strong>Assembly</strong> — selected scenes plus title/end cards are concatenated with the genre's signature transition (see table below).</li>
  ...
  <li><strong>Narration (optional)</strong> — Fish Audio S2 (self-hosted or cloud) generates speech from a script in a voice cloned from <code>weng_voice.wav</code>, or an uploaded VO track is used; either way it's loudness-normalized before mixing.</li>
...
<p>Jobs run through a concurrency-limited queue (default 2 at a time, configurable via <code>/api/queue/limit</code>) so multiple simultaneous requests don't overload the local model servers (ACE-Step, Woosh, Ollama, Fish Audio).</p>
...
function runHealthCheck(){
  ...
  fetch('/api/health').then(r=>r.json()).then(d=>{
    ...
    var fishSvc = (d.services || []).find(function(s){ return s.name === 'fish_audio' })
    if(fishSvc && fishSvc.status === 'up' && typeof loadFishAudioVoices === 'function') loadFishAudioVoices()
  }).catch(function(e){
...
// "Narration (Fish Audio)" options are already greyed out if their service is
