...
TTS_URL = 'http://localhost:8020'  # optional local TTS server (e.g. a Piper/XTTS HTTP wrapper); falls back to pyttsx3 if unreachable
...
def job_set(jid, percent=None, step=None, error=None, done=None, result=None):
    with JOBS_LOCK:
        j = JOBS.get(jid)
        if not j:
            return
...
def generate_tts(text, output_wav_path, rate=175, voice_id=None):
    """Generate a voiceover WAV from text. Tries a local TTS HTTP server first
    (same pattern as ACE-Step), then falls back to pyttsx3 (fully offline,
    uses SAPI5/NSSpeechSynthesizer/espeak depending on OS). Returns (ok, error_message)."""
    text = (text or '').strip()
    if not text:
    ...
    try:
        ...
        pass  # local TTS server not running — fall through to pyttsx3
    try:
        import pyttsx3
    except ImportError:
        return False, 'No TTS backend available (local TTS server unreachable and pyttsx3 is not installed — pip install pyttsx3)'
    try:
        engine = pyttsx3.init()
        ...
        return ok, (None if ok else 'TTS engine produced no audio (on Linux this needs espeak/espeak-ng installed)')
    except Exception as e:
...
def api_trailer():
    ...
    params = dict(path=path, orig_name=orig_name, mode=mode, genre=genre, scoring_mode=scoring_mode,
                  trailer_length=trailer_length, transition=transition, xfade_dur=xfade_dur,
                  target_loudness=target_loudness, true_peak=true_peak, beat_match=beat_match, model=model,
                  sfx_mode=sfx_mode, sfx_upload_path=sfx_upload_path, jl_cut=jl_cut, jl_offset=jl_offset,
                  vo_mode=vo_mode, vo_upload_path=vo_upload_path, vo_text=vo_text, vo_voice=vo_voice,
                  vo_rate=vo_rate, vo_start=vo_start, sync_beats=sync_beats,
                  end_card_path=end_card_path, schedule_card_path=schedule_card_path,
                  scoring_audio_path=scoring_audio_path, prompt=prompt)
    threading.Thread(target=run_trailer_job, args=(jid, params), daemon=True).start()
    return jsonify(job_id=jid)

@app.route('/api/tts/voices')
def api_tts_voices():
    return jsonify(voices=list_tts_voices())
...
def _run_trailer_job(jid, params):
    ...
    jl_cut = params['jl_cut']; jl_offset = params['jl_offset']
    ...
    if video_duration < min_required:
        job_set(jid, error=f'Video is only {video_duration:.1f}s long, but a {trailer_length}s trailer requires at least {min_required:.1f}s of raw video. Upload a longer video or select a shorter trailer length.')
        return
    ...
    for pass_attempt in range(3):
        ...
        for s in scenes_data:
            ...
            if sync_beats and beat_times:
                ...
                snapped_cut = nearest_beat(target_cut, beat_times, total_sel + 0.3, total_sel + s['duration'])
                seg_dur = max(0.3, min(s['duration'], snapped_cut - total_sel))
    ...
    if not os.path.exists(out_path) or os.path.getsize(out_path) == 0:
        job_set(jid, error='Trailer generation failed (ffmpeg output empty).')
        return
    ...
    if v.returncode != 0 or float(v.stdout.strip() or 0) <= 0:
        job_set(jid, error='Trailer generation failed (invalid/corrupt output).')
        return
    ...
    job_set(jid, percent=70, step='Applying J/L cuts')
    ...
    # Two improvements over a plain overlay: (1) each pad gets a short fade
    # in/out so it never clicks against the extracted-clip audio, and (2) the
    # underlying track is briefly ducked during the pad window so the bled-in
    # audio actually reads as "early/lingering dialogue" instead of just mud.
    pad_files = []
    if jl_cut != 'none' and len(selected) > 1:
        ...
        for j, (delay_s, src_start, dur) in enumerate(pad_jobs):
            ...
            fade_len = min(0.03, dur / 3)
            afilter = f'afade=t=in:d={fade_len},afade=t=out:st={max(dur - fade_len, 0)}:d={fade_len}'
            r = subprocess.run([FFMPEG, '-y', '-ss', str(src_start), '-i', path,
                                '-t', str(dur), '-vn', '-af', afilter,
                                '-c:a', 'aac', '-b:a', '192k', pad_path],
                               capture_output=True, text=True, timeout=30)
            if os.path.exists(pad_path) and os.path.getsize(pad_path) > 0:
                pad_files.append((delay_s, dur, pad_path))

        if pad_files:
            jl_mixed = os.path.join(app.config['UPLOAD_FOLDER'], f'jlmix_{base_ts}.mp4')
            cmd = [FFMPEG, '-y', '-i', out_path]
            for _, _, pf in pad_files:
                cmd.extend(['-i', pf])
            fc_parts = []
            # Duck the base track (slightly, briefly) under every pad window so the
            # bled-in audio doesn't just layer on top of a full-volume base track.
            duck_expr = '+'.join(
                f"between(t\\,{max(delay_s,0):.3f}\\,{delay_s+dur:.3f})*0.55"
                for delay_s, dur, _ in pad_files
            )
            fc_parts.append(f"[0:a]volume='1-({duck_expr})':eval=frame[base]")
            mix_labels = ['base']
            for idx, (delay_s, dur, _) in enumerate(pad_files):
    ...
    # Voiceover: upload or TTS, ducking everything else (dialogue + music + SFX)
    # underneath it wherever it plays, the same sidechain-compression technique
    # used for music-under-dialogue above.
    vo_source = 'none'  # 'uploaded' | 'tts' | 'none'
    vo_error = None
    if vo_mode != 'none':
        job_set(jid, percent=90, step='Adding voiceover' + (' (generating speech)' if vo_mode == 'tts' else ''))
        vo_raw_path = None
        if vo_mode == 'upload' and vo_upload_path and os.path.exists(vo_upload_path):
            ...
            ok, err = generate_tts(vo_text, tts_wav, rate=vo_rate, voice_id=vo_voice)
            if ok:
                vo_raw_path = tts_wav
                vo_source = 'tts'
            else:
        ...
        if vo_raw_path:
            vo_m4a = os.path.join(app.config['UPLOAD_FOLDER'], f'vo_{base_ts}.m4a')
            ms = max(0, int(vo_start * 1000))
            r = subprocess.run([FFMPEG, '-y', '-i', vo_raw_path,
                                '-af', f'adelay={ms}|{ms},volume=1.15',
                                '-c:a', 'aac', '-b:a', '192k', vo_m4a],
                               capture_output=True, text=True, timeout=30)
            if os.path.exists(vo_m4a) and os.path.getsize(vo_m4a) > 0:
                vo_mixed = os.path.join(app.config['UPLOAD_FOLDER'], f'vomix_{base_ts}.mp4')
                r = subprocess.run([FFMPEG, '-y', '-i', out_path, '-i', vo_m4a,
                                    '-filter_complex',
                                    '[0:a]asplit[bed][side];[side]volume=1.0[sc];'
                                    '[bed][sc]sidechaincompress=threshold=0.05:ratio=8:attack=30:release=500:makeup=1[bed_ducked];'
                                    f'[bed_ducked][1:a]amix=inputs=2:duration=first:dropout_transition=2:normalize=0,'
                                    f'loudnorm=I={target_loudness}:TP={true_peak}:LRA=7[outa]',
                                    '-map', '0:v', '-map', '[outa]',
                                    '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k',
                                    '-shortest', vo_mixed],
                                   capture_output=True, text=True, timeout=120)
                if os.path.exists(vo_mixed) and os.path.getsize(vo_mixed) > 0:
                    os.replace(vo_mixed, out_path)
                else:
                    print(f'VO mix error: {r.stderr[:500]}')
            if os.path.exists(vo_m4a):
                os.remove(vo_m4a)
...
<form id=tf method=POST action=/api/trailer/generate enctype=multipart/form-data>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin:8px 0">
    ...
    <label style="font-size:13px;text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:4px;margin:0;cursor:pointer">
      <input type=radio name=vo_mode value=tts> Generate (text-to-speech)
    </label>
  </div>
  ...
  <div id=vo-tts-area style="display:none">
    <textarea name=vo_text rows=3 style="width:100%;box-sizing:border-box" placeholder="Type the voiceover script here..."></textarea>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:8px 0">
      <span style="font-size:12px">Voice:</span>
      <select name=vo_voice id=vo-voice-select style="width:auto"><option value="">Default</option></select>
      <span style="font-size:12px">Rate (wpm):</span>
      <input type=number name=vo_rate value=175 min=80 max=300 step=5 style="width:80px">
    </div>
    <p style="margin-top:4px;font-size:12px;opacity:.75">Uses an offline TTS engine (no cloud calls). If none is installed, this will report an error instead of silently failing.</p>
  </div>
  <div id=vo-common-area style="display:none;margin:8px 0">
    <span style="font-size:12px">Start at (s into trailer):</span>
    <input type=number name=vo_start value=0 min=0 step=0.5 style="width:80px">
    <p style="margin-top:4px;font-size:12px;opacity:.75">Music, SFX, and original dialogue automatically duck under the voiceover wherever it plays.</p>
  </div>
  ...
  fetch('/api/tts/voices').then(r=>r.json()).then(d=>{
    var sel=document.getElementById('vo-voice-select')
    ;(d.voices||[]).forEach(v=>{
...
<div id=tr-progress-area style="display:none;margin:14px 0">
...
document.getElementById('tf').addEventListener('submit', async function(e){
  ...
  var srcLabel=function(s){return {ai_generated:'AI-generated',uploaded:'uploaded',synth_fallback:'placeholder synth (ACE-Step unavailable)',tts:'text-to-speech',none:'none'}[s]||s}
  var audioNote=''
